...
`docs/manual/00-index.md`, `01-explanation.md`, `02-howto-create-variant.md`, `03-howto-attach-existing.md`, `04-howto-reorder-variants.md`, `05-howto-remove-variant.md`, `06-reference-rules.md`, `07-reference-glossary.md`, `08-known-limitations.md`. Plus `_assets/` (17 published screenshots).
...
| A.4 | Every screenshot referenced in `docs/manual/*.md` has a corresponding row in `09-shotlist.md` and a file in `_assets/`. | PASS | Manual references shots 03, 05, 06, 07, 08, 09, 10, 11, 13, 15, 17, 18, 19, 23, 31, 33, 34 — all listed in `09-shotlist.md` and present in `_assets/`. | `09-shotlist.md`, `ls _assets/` |
...
| # | Check | Verdict | Rationale | Evidence |
|---|---|:-:|---|---|
| E.1 | Every image has alt text in its Markdown image syntax, and that text is non-empty and descriptive. | PASS | Checked all 22 image references across 6 files; each carries a descriptive alt text different from the caption. | `docs/manual/*.md` |
...
| F.5 | Known-Limitations entries are cross-linked from places where the gap is felt. | PASS | UK7 referenced from R6.3 (kitchen ticket); UK8 from R2 + 02-howto Notes (`Short Name` constraints); UK9 from R6.1 + 04-howto Notes; UK10 from R7 + R8. Gaps that are not triggered inside a specific procedure remain centralized in `08-known-limitations.md`. | `06-reference-rules.md`, how-tos |

## G. Safe-scope publication (DoD §3.2 — ISO 26514 + spec)

| # | Check | Verdict | Rationale | Evidence |
|---|---|:-:|---|---|
| G.1 | Body of the manual contains only D, D+N, or PO-confirmed NN claims. | PASS | C6b (PO-confirmed NN) is the only NN-only claim in the body, framed as authoritative + flagged in 05-howto Notes. C14 and C16 keep only neutral structural wording in the body; risky layout/behaviour detail is deferred to Limitations. | RTM Part 1 + `06-reference-rules.md` |
